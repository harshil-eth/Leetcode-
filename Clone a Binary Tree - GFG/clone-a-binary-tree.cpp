...
private:
    ...
    Node* copy_normal(Node* tree, unordered_map<Node*, Node*> &mp) {
        if(!tree) 
            return NULL;
            
        mp[tree] = new Node(tree->data);
        mp[tree]->left = copy_normal(tree->left, mp);
        mp[tree]->right = copy_normal(tree->right, mp);
        return mp[tree];
    }
    
    void cloned(Node* root, unordered_map<Node*, Node*> &mp) {
        if(!root) 
            return;
            
        mp[root]->random = mp[root->random];
        cloned(root->left, mp);
        cloned(root->right, mp);
